...
Handsontable.Core = function (rootElement, userSettings) {
  ...
  var $document = jQuery(document.documentElement);
 ...
 *   * @param {Number} [amount] Optional.
  ...
  this.destroy = function () {
    ...
    jQuery(window).off('.' + instance.guid);
...
Handsontable.TableView = function (instance) {
  var that = this
    ...
    , $documentElement = jQuery(document.documentElement);
  ...
  $documentElement.on('mousedown.' + instance.guid, function (event) {
    ...
    if (next !== that.wt.wtTable.spreader) { //immediate click on "spreader" means click on the right side of vertical scrollbar
      while (next !== document.documentElement) {
        ...
        if (next === instance.rootElement[0] || next.nodeName === 'X-HANDSONTABLE' || next.id === 'context-menu-layer' || jQuery(next).is('.context-menu-list') || jQuery(next).is('.typeahead li')) {
  ...
  jQuery(that.wt.wtTable.spreader).on('mousedown.handsontable, contextmenu.handsontable', function (event) {
           ...
           for(var col = selEnd.col; col <= selEnd.col; col++){
             ...
             if(checkbox.length > 0 && !cellProperties.readOnly){
               for(var i = 0, len = checkbox.length; i < len; i++){
                 ...
                 jQuery(checkbox[i]).trigger('change');
               }
             }
...
HandsontableTextEditorClass.prototype.refreshDimensions = function () {
  ...
  var $td = jQuery(this.TD); //because old td may have been scrolled out with scrollViewport
...
HandsontableTextEditorClass.prototype.discardEditor = function (result) {
  ...
  if (result === false && this.cellProperties.allowInvalid !== true) { //validator was defined and failed
    ...
    if (this.waitingEvent) { //this is needed so when you finish editing with Enter key, the default Enter behavior (move selection down) will work after async validation
      ...
      this.waitingEvent = null;
      jQuery(document.activeElement).trigger(ev);
    }
  }
};
...
HandsontableDateEditorClass.prototype.createElements = function () {
  ...
  this.$datePicker = jQuery(this.datePicker);
...
HandsontableDateEditorClass.prototype.showDatepicker = function () {
  var $td = jQuery(this.instance.dateEditor.TD);
...
(function(Handsontable){
  ...
  function destroyContextMenu() {
     ...
     */
    //jQuery(document.querySelectorAll('#context-menu-layer')).remove();
  }
  ...
  var bindMoveColEvents = function () {
    ...
    instance.rootElement.on('mouseup.manualColumnMove', function () {
      if (pressed) {
        ...
        jQuery('.manualColumnMover.active').removeClass('active');
        pressed = false;
        instance.forceFullRender = true;
...
(function($){
  ...
  function autoResize(config) {
    this.filter(autoResize.resizableFilterSelector).each(function(){
      new AutoResizer( jQuery(this), config );
    });
    return this;
  }
  ...
  AutoResizer.prototype = {
    ...
    createClone: function() {
      ...
      if (config.minWidth !== 'original' || config.maxWidth !== 'original') {
        this.wClone = jQuery('<div/>').width('auto').css({
          whiteSpace: 'nowrap',
          'float': 'left'
        });
    ...
    injectClone: function() {
      (
        autoResize.cloneContainer ||
          (autoResize.cloneContainer = jQuery('<arclones/>').appendTo('body'))
...
CopyPasteClass.prototype._bindEvent = (function () {
  if (window.jQuery) { //if jQuery exists, use jQuery event (for compatibility with $.trigger and $.triggerHandler, which can only trigger jQuery events - and we use that in tests)
    return function (elem, type, cb) {
      jQuery(elem).on(type + '.copypaste', cb);
    };
  }
  else {
    return function (elem, type, cb) {
...
//  style.visibility = 'hidden';
  ...
  /*jQuery(this.top).on(sss, function(event) {
   event.preventDefault();
   event.stopImmediatePropagation();
   jQuery(this).hide();
   });
   jQuery(this.left).on(sss, function(event) {
   event.preventDefault();
   event.stopImmediatePropagation();
   jQuery(this).hide();
   });
   jQuery(this.bottom).on(sss, function(event) {
   event.preventDefault();
   event.stopImmediatePropagation();
   jQuery(this).hide();
   });
   jQuery(this.right).on(sss, function(event) {
   event.preventDefault();
   event.stopImmediatePropagation();
   jQuery(this).hide();
   });*/
  ...
  jQuery(this.main.childNodes).on('mouseenter', function (event) {
...
Walkontable.prototype.destroy = function () {
  jQuery(document.body).off('.' + this.guid);
...
/*  var lastMouseOut;
  ...
  jQuery(this.instance.wtTable.holder).on('mousedown', onMouseDown);
  jQuery(this.instance.wtTable.TABLE).on('mouseover', onMouseOver);
//  jQuery(this.instance.wtTable.TABLE).on('mouseout', onMouseOut);
  jQuery(this.instance.wtTable.holder).on('mouseup', onMouseUp);
}
...
WalkontableScrollbar.prototype.init = function () {
  ...
  //reference to instance
  this.$table = jQuery(this.instance.wtTable.TABLE);
...
WalkontableScrollbarNative.prototype.init = function () {
  ...
  this.$scrollHandler = jQuery(window); //in future remove jQuery from here
...
function WalkontableWheel(instance) {
  ...
  jQuery(instance.wtTable.spreader).on('mousewheel', function (event, delta, deltaX, deltaY) {
...
var // currently active contextMenu trigger
    ...
    handle = {
        ...
        click: function(e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            jQuery(this).trigger($.Event("contextmenu", { data: e.data, pageX: e.pageX, pageY: e.pageY }));
        },
        ...
        mouseenter: function(e) {
            var $this = jQuery(this),
                $related = jQuery(e.relatedTarget),
        ...
        focusInput: function(e) {
            var $this = jQuery(this).closest('.context-menu-item'),
                data = $this.data(),
    ...
    op = {
        ...
        create: function(opt, root) {
            ...
            // create contextMenu
            opt.$menu = jQuery('<ul class="context-menu-list"></ul>').addClass(opt.className || "").data({
                'contextMenu': opt,
            ...
            $.each(opt.items, function(key, item){
                var $t = jQuery('<li class="context-menu-item"></li>').addClass(item.className || ""),
                    $label = null,
                    $input = null;
                ...
                if (typeof item == "string") {
                    ...
                    if (item.type == 'html') {
                        ...
                        $label = jQuery('<label></label>').appendTo($t);
                        jQuery('<span></span>').html(item._name || item.name).appendTo($label);
                    ...
                    switch (item.type) {
                        case 'text':
                            $input = jQuery('<input type="text" value="1" name="" value="">')
                        ...
                        case 'textarea':
                            $input = jQuery('<textarea name=""></textarea>')
                                .attr('name', 'context-menu-input-' + key)
                                .val(item.value || "")
                                .appendTo($label);
                        ...
                        case 'checkbox':
                            $input = jQuery('<input type="checkbox" value="1" name="" value="">')
                                .attr('name', 'context-menu-input-' + key)
                                .val(item.value || "")
                        ...
                        case 'radio':
                            $input = jQuery('<input type="radio" value="1" name="" value="">')
                        ...
                        case 'select':
                            $input = jQuery('<select name="">')
                                .attr('name', 'context-menu-input-' + key)
                                .appendTo($label);
                            if (item.options) {
                                $.each(item.options, function(value, text) {
                                    jQuery('<option></option>').val(value).text(text).appendTo($input);
                                });
                        ...
                        case 'sub':
                            ...
                            jQuery('<span></span>').html(item._name || item.name).appendTo($t);
        ...
        layer: function(opt, zIndex) {
            ...
            var $layer = opt.$layer = jQuery('<div id="context-menu-layer" style="position:fixed; z-index:' + zIndex + '; top:0; left:0; opacity: 0; filter: alpha(opacity=0); background-color: #000;"></div>')
...
$.contextMenu = function(operation, options) {
    ...
    if (!o.context || !o.context.length) {
        ...
        $context = jQuery(o.context).first();
        ...
        case 'destroy':
            ...
            if (_hasContext) {
                ...
                $.each(menus, function(ns, o) {
                    ...
                    $visibleMenu = jQuery('.context-menu-list').filter(':visible');
                    if ($visibleMenu.length && $visibleMenu.data().contextMenuRoot.$trigger.is(jQuery(o.context).find(o.selector))) {
